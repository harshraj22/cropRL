...
class TestMultiHarvestCycles:
    ...
    def test_two_harvest_cycles(self):
        ...
        e.step(CroprlAction(action_id=0))  # wait (age → 1)
        obs1 = e.step(CroprlAction(action_id=7))  # harvest & sell
        assert obs1.active_crop_type == 0  # fallow
        cash_after_1 = obs1.cash_balance

        # Cycle 2: Plant corn on the now-fallow land
        obs_plant = e.step(CroprlAction(action_id=1))  # plant corn
        assert obs_plant.active_crop_type == 1
        for _ in range(3):
            e.step(CroprlAction(action_id=0))  # grow corn
        obs2 = e.step(CroprlAction(action_id=7))  # harvest & sell
    ...
    def test_three_cycle_rotation(self):
        ...
        for crop_id in crops:
            e.step(CroprlAction(action_id=crop_id))  # plant
            e.step(CroprlAction(action_id=0))  # wait
            obs = e.step(CroprlAction(action_id=7))  # harvest
    ...
    def test_store_then_plant_next(self):
        ...
        e.step(CroprlAction(action_id=3))
        ...
        obs = e.step(CroprlAction(action_id=8))  # sell inventory
    ...
    def test_cash_increases_over_profitable_cycle(self):
        ...
        for _ in range(3):  # grow to maturity
            e.step(CroprlAction(action_id=0))
        obs = e.step(CroprlAction(action_id=7))
